...
# ── Step 1: Auto-discover ALL databases from PROD ─────────────
echo "Discovering databases from PROD..."
DISCOVERED=$(psql -h $PROD_HOST -p $DB_PORT -U $DB_USER -t -A -c \
    "SELECT datname FROM pg_database
     WHERE datname NOT IN ('template0','template1','postgres',
                           'keycloak','n8n','superset',
                           'homer_config','homer_data')
...
# Convert to array
mapfile -t DATABASES <<< "$DISCOVERED"
